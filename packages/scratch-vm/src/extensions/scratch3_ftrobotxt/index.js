/*
    ...
    Workaround for the txt-controller. Since the controller only has a raw
    tcp-Socket-Server capable of writing and reading inputs and outputs, we
    use an implemented websocket server. It has access to the file
    directory and the txt firmware can read and compile sb3 files, so
    scratch programs for the txt have to be downloaded to the pc and then
    uploaded to the txt.
    A faster way would connect directly to the txt webserver and access
    the directory, but the current authorisation requirements block that.
*/
...
/* global controller, translate */

const outInt = 0; // number of outputs *3 --> 4 for each individual output 2 for each motor set
