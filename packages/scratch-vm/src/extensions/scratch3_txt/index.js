/*
    ...
    Once the TXT extension is opened, a connection can be established to a
    fischertechnik TXT controller by clicking the orange connect button.
    Then, hold the red "Select" button on the TXT until the blinking blue
    LED blinks with a much higher frequency. You should see the right
    controller now in the bluetooth connection window of the browser.
    Select and pair our controller. Wait until the LED on the TXT turns
    orange. Depending on whether you allowed notifications, you will either
    receive a notification or an alert when the connection is finished and
    the controller ready to be used.
...
class Scratch3TXTBlocks {
    constructor (runtime) {
        ...
        globalThis.openedextensions.push('TXT');
    ...
    getCounter (args) { // COUNTER_ID
        b.getCounter(args, globalThis.controller);
    }
