/*
  scratch3_robby/index.js
    ...
    extension called BT-Smart is opened a connection can be established to a
    fischertechnik BT-Smart controller by clicking the orange connect button. Then,
    holt the red "Select"-button on the BT-Smart until the blinking blue LED blinks
    ...
    Wait until the LED on the BT-Smart turns orange. Depending on whether you
    allowed notifications, you will either receive a notification or an alert when
    the connection is finished and the controller ready to be used.
...
const maxSpeed = 6; // max speed for motors
...
class Scratch3RobbyBlocks {
    constructor (runtime) {
        ...
        globalThis.openedextensions.push('Robby');
        if (globalThis.extensionnumber > 1) {
            main.addselections();
        } else {
            globalThis.type = 'Robby';
        }
        main.addButton();
