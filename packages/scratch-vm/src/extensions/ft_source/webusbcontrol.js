/* eslint-disable eqeqeq, no-undefined, no-undef, radix, @stylistic/max-len, @stylistic/no-mixed-operators, no-console, func-style, jsdoc/require-param-description, jsdoc/require-param-type, jsdoc/no-undefined-types, jsdoc/require-returns, block-scoped-var, no-shadow, require-await, no-var, no-redeclare, no-dupe-class-members, no-return-assign, prefer-promise-reject-errors, no-unused-vars, no-empty, no-async-promise-executor */
require('core-js');
...
class LT{
    ...
    async readInput (result) {
        ...
        const d1 = digitalInputs & 0x01;
        const d2 = digitalInputs & 0x02;
        const d3 = digitalInputs & 0x04;
    ...
    getwriteOut (ind, val){
        ...
        if (ind === 0 || ind === 1) { // M1, M2
            ...
            const oIndex = ind === 0 ? [0x01, 0x02] : [0x04, 0x08];
            if (val > 0) {
                byte1 |= oIndex[1];
                byte1 &= ~oIndex[0];
            ...
            // }
            const mask = 1 << (ind - 2);
            const isO1 = ind === 2; const isO2 = ind === 3; const isO3 = ind === 4; const isO4 = ind === 5;
    ...
    getwriteOut2 (ind, val){
        ...
        const previousSequence = [0x00, 0x00, 0x00, 0x00];
        ind = ind + 1;
        if (val > 0){
            dir = 1;
        } else {
        ...
        const byte2 = (pwmValue << 4) | pwmValue | previousSequence[2]; // speed 01 and 02
    ...
    getwriteOut3 (ind, val, state = true){
        const pwm = [0, 0, 0, 0];
        const enable = [false, false, false, false];
        // constants for motor direction states
        const Off = 0;
        const Left = 1;
        const Right = 2;
        const Brake = 3;
        const pwm2 = Math.round((val / 255) * 100);
...
class ftduino{
    ...
    async readInput (indee) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const res = textDecoder.decode(result.data);
    ...
    getwriteLED (val){
        textEncoder = new TextEncoder();
        const data = this.textEncoder.encode(JSON.stringify({set: {port: 'led', value: val}}));
        return data;
    }
}
...
class WebUSBDevice{
    ...
    async connect (){
        ...
        return connect = new Promise((resolve, reject) => {
            const filter = [{vendorId: type.vendorId, productId: type.productId}];
            console.log(filter);
