/* eslint-disable eqeqeq, no-undefined, no-undef, @stylistic/max-len, @stylistic/no-mixed-operators, no-console, func-style, jsdoc/require-param-description, jsdoc/require-param-type, jsdoc/no-undefined-types, jsdoc/require-returns, block-scoped-var, no-var, no-redeclare, no-negated-condition, no-unused-vars, no-invalid-this, no-use-before-define, no-empty, no-eq-null */
...
let count = 0;
buttonpressed = false;
const controllerknown = false;
let connection = 'BLE';
let notis; // Permission and API supported--> 0 cant be used(not granted or supported); 1 API supported; 2 supported and Permission granted--> can be used
img = undefined; // img
// img
    ...
    if (controller == undefined){
        navigator.serial.getPorts({}).then(ports => {
            if (ports.length > 0){
                ...
                count = 20;
                if (controller != undefined){
                    const img = document.getElementById(FT_BUTTON_ID);
                    ...
                    controller.autoconnect().then(device => { // Connect function is async--> then
                        ...
                        .catch(error => {
                            ...
                            } else if (openedextensions.includes('LT')){
                                type = 'LT';
...
async function stud () { // function of connect button
    const img = document.getElementById(FT_BUTTON_ID); // Ensure img is defined
...
class Main {
    ...
    setButton (state, msg = null) { // Function which changes the button
        const button = document.getElementById(FT_BUTTON_ID);
        if (button) {
    ...
    addButton (initial = true) { // Function which creates the button
        ...
        if (!button) {
            const x = document.getElementsByClassName(PARENT_CLASS);
            if (x.length > 0) {
                // var x;
                // x[0]=1;
                const hdrdiv = x[0];
    ...
    addselections () {
        ...
        select.addEventListener('change', () => { // Eventlistener for the selection
