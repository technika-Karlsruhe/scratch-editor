/* eslint-disable eqeqeq, no-undefined, no-undef, radix, @stylistic/max-len, @stylistic/no-mixed-operators, no-console, func-style, jsdoc/no-undefined-types, jsdoc/require-returns, block-scoped-var, no-shadow, require-await, no-var, no-redeclare, no-return-assign, no-negated-condition, no-unused-vars, no-use-before-define, no-empty, no-lonely-if */
require('core-js');
...
let f = 0;
let s = 0;
const charWrite = new Array(); // chars of all writable (0, 1 --> Motor; 2-5--> Inputs)
const charZust = new Array(); // Represents pending promises--> first two for M1; M2 next four IModes 1-4 -->0: no promise pending, characteristic can be written, | 1: wait until the promise is resolved
const stor = new Array(); // memory
const charI = new Array();
let serviceOut;
let serviceIn;
let serviceIMode;
let serviceLED;
...
const input = { // event handler; if a controller with more inputs is added, further input functions have to be added
    in_0: function (event){
        if (type.name == 'Robby') {
            const controllerValue = event.target.value.getUint8(0);
            // Check if button 1 is pressed
            const isButton1Pressed = (controllerValue & 0x01) !== 0;
            valIn[6] = isButton1Pressed ? 0 : 255;
            // Check if button 2 is pressed
            const isButton2Pressed = (controllerValue & 0x02) !== 0;
            valIn[7] = isButton2Pressed ? 0 : 255;
            // Check if trail sensor 1 has been triggered
            const isSensor1Triggered = (controllerValue & 0x10) !== 0;
            valIn[8] = isSensor1Triggered ? 0 : 255;
            // Check if trail sensor 2 has been triggered
            const isSensor2Triggered = (controllerValue & 0x20) !== 0;
...
function connectIMo (){ // connection of IModes
    if (type.name === 'RXC'){
        ...
        const setupByte = (rxMode << 3) | 0x40; // Type on Bits 3-5, Bit 6 for all Inputs
        characteristic = serviceIMode.getCharacteristic(type.uuidsIM[0]).then(characteristic => characteristic.writeValue(new Uint8Array([setupByte])))
            .then(() => {
...
class BLEDevice {
    ...
    connecthand (){ // wait until all features have been initialized
        const expectedOutCount = (type.name === 'RXC') ? type.indOut : type.indOut / 3;
    ...
    write (ind){ // actual write method
        ...
        if (valWrite[ind] == stor[ind][0]){ // if we would write the same value again we can skip it in order to not block the connection
        ...
        } else if (charZust[ind] == 0 && stor[ind].length > 0){ // if nothing is being changed and storage is not empty
            const val = stor[ind][0]; // we have to save the value, if the storage is cleared while the write command is executed valWrite might receive a false value
            ...
            if (ind < type.indOut){ // an output value has to be changed
                if (type.name === 'RXC') {
                    ...
                    const scaled = Math.round(stor[ind][0] * 512 / 127);
                    const value = Math.max(-512, Math.min(512, scaled));
                    const buffer = new ArrayBuffer(2);
                    const view = new DataView(buffer);
                    view.setInt16(0, value, true); // little endian
                    if (valWrite[ind] == stor[ind][0] || valWrite[ind] == 0 || stor[ind][0] == 0){
                        // no stop needed
                        waitForBleFree().then(() => charWrite[ind].writeValue(new Uint8Array(buffer)))
                            .then(x => {
                                bleWriteBusy = false;
                                valWrite[ind] = val;
                                charZust[ind] = 0;
                        ...
                        waitForBleFree().then(() => {
                            // Stop: 2 Byte with 0
                            const stopBuffer = new ArrayBuffer(2);
                            ...
                            .then(() => waitForBleFree().then(() => charWrite[ind].writeValue(new Uint8Array(buffer))))
                            .then(x => {
                                bleWriteBusy = false;
                                valWrite[ind] = val;
                                charZust[ind] = 0;
                    ...
                    if (valWrite[ind] == stor[ind][0] || valWrite[ind] == 0 || stor[ind][0] == 0){
                        // no stop needed
                        waitForBleFree().then(() =>
                            charWrite[ind].writeValue(new Uint8Array([writeValue])) // writeValue [stor[ind][0]]
                        )
                            .then(x => {
                                bleWriteBusy = false;
                                valWrite[ind] = val;
                        ...
                        // stop needed
                        waitForBleFree().then(() => charWrite[ind].writeValue(new Uint8Array([0])))
                            .then(x => {
                                bleWriteBusy = false;
                                if (ismobile()) {
                                    return new Promise(resolve => setTimeout(resolve, 100));
                            ...
                            .then(() =>
                                waitForBleFree().then(() => charWrite[ind].writeValue(new Uint8Array([writeValue]))) // writeValue [stor[ind][0]]
                            )
                            .then(x => {
                                bleWriteBusy = false;
                                valWrite[ind] = val;
                ...
                const getLedCharacteristic = () => serviceLED.getCharacteristic(type.uuidLED);
                const getLedPromise = ismobile() ?
                ...
                if (type.name === 'RXC') {
                    ...
                    const rxMode = RX_INPUT_MODE_MAP[stor[ind][0]] !== undefined ? RX_INPUT_MODE_MAP[stor[ind][0]] : stor[ind][0];
                    // console.log("RX input mode:", rxMode);
                    const setupByte = ((ind - 12) & 0x07) | ((rxMode & 0x07) << 3); // Port + type
                    // console.log("input: "+ind + " targetMode: " + stor[ind][0] + " input-12: "+(ind-12));
                    // all inputs: setupByte |= 0x80
                    waitForBleFree().then(() => serviceIMode.getCharacteristic(type.uuidsIM[0]).then(characteristic => characteristic.writeValue(new Uint8Array([setupByte]))))
                        .then(() => {
                            bleWriteBusy = false;
                    ...
                    waitForBleFree().then(() => charWrite[ind].writeValue(new Uint8Array([stor[ind][0]])))
                        .then(x => {
                            bleWriteBusy = false;
                            charZust[ind] = 0;
                            valWrite[ind] = val;
        ...
        if (funcstate[parseInt(args.INPUT)] == 0){ // function is called for the first time
            ...
            charI[parseInt(args.INPUT)].stopNotifications().then(x => { // no unwanted signal
                ...
                charWrite[parseInt(args.INPUT)].writeValue(new Uint8Array([val])).then(x =>
                    charI[parseInt(args.INPUT)].readValue() // Reading a Value with the new Input mode (to avoid an "old value" being stored)
                )
                    .then(x =>
                        charI[parseInt(args.INPUT)].startNotifications()// Notifications are enabled again
                    )
                    .then(x =>
                        charI[parseInt(args.INPUT)].readValue() // to ensure that we don't read a false value, we read the current value
                    )
                    .then(x => {
    ...
    changeInMode (args) { // Called by Hats to handle wrong input modes
        ...
        if (type.name === 'RXC') {
            if (funcstate[input] == 0) { // function is called for the first time
                ...
                // console.log("RX input mode:", rxMode);
                const setupByte = ((input - 12) & 0x07) | ((rxMode & 0x07) << 3); // Port + type
        ...
        } else if (funcstate[input] == 0) { // function is called for the first time
            funcstate[input] = 1;
            ...
            charI[input].stopNotifications().then(() => {
                ...
                .then(() => charI[input].readValue())
                .then(() => charI[input].startNotifications())
                .then(() => charI[input].readValue())
                .then(() => {
                    valWrite[input] = targetMode;
                    charZust[input] = 0;
